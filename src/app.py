...
# Unregister endpoint
@app.post("/activities/{activity_name:path}/unregister")
def unregister_from_activity(activity_name: str, email: str):
    """Remove a student from an activity"""
    print(f"DEBUG: Received activity_name='{activity_name}', email='{email}'")
    if activity_name not in activities:
        print(f"DEBUG: Activity '{activity_name}' not found in activities.")
        raise HTTPException(status_code=404, detail="Activity not found")
    activity = activities[activity_name]
    if email not in activity["participants"]:
        print(f"DEBUG: Email '{email}' not registered for activity '{activity_name}'.")
        raise HTTPException(status_code=400, detail="Student not registered for this activity")
    activity["participants"].remove(email)
    print(f"DEBUG: Removed '{email}' from '{activity_name}'.")
